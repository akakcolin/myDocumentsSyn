...
#set key right
set xlabel 'NSW'
set ylabel 'Log(Total energy (KJ/mol)- E_{min})'
...
files=system("ls -1B  sp-opt-nei   | sort")
do for [ file in files ] {
    o1 = sprintf('sp-opt-nei/%s/log', file)
    o2 = sprintf('sp-opt/%s/log', file)
    set output "temp.tex"
    ...
    set output out
    t1 = sprintf('%s-gadget', file)
    t2 = sprintf('%s-normal', file)
    #plot o1 using (log($2*ev2kj-min_y)) w lp ls 3 pt 4 t t1, o2 using (log($2*ev2kj-min_y)) w lp ls 4 pt 5 t t2
    plot o1 using (log($2*ev2kj-min_y)) w lp ls 3 pt 4 t t1, o2 using (log($2*ev2kj-min_y)) w lp ls 4 pt 5 t t2
